...
<head>
    ...
    <style>
        /* CSS for the modal and table styling */
...
<script>
    ...
    function handleCreate(event) {
        ...
        const formData = {
            ...
            denda: parseInt(document.getElementById('denda').value.replace(/\D/g, '')) || 0
        };
        ...
        .then(data => {
            if (data.success) {
                ...
                const newRow = `<tr id="item-${item.id}">
                    <td>${item.nama_peminjam}</td>
                    <td>${item.nama_buku}</td>
                    <td>${item.jumlah_buku}</td>
                    <td>${item.tanggal_pinjam}</td>
                    <td>${item.tanggal_kembali || 'Not Returned'}</td>
                    <td>Rp ${new Intl.NumberFormat('id-ID').format(item.denda)}</td>
                    <td>
                        <button class="action-btn" onclick="editItem(${item.id})">Edit</button>
                        <button class="delete-btn" onclick="deleteItem(${item.id})">Delete</button>
                    </td>
    ...
    function editItem(id) {
        // Fetch item details, populate form, and show edit modal
