    ...
    $(document).ready(function() {
        ...
        $('#createBtn').on('click', function() {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#itemsTable').append(`
                        <tr data-id="${response.item.id}">
                            ...
                            <td>${response.item.tanggal_kembali || 'N/A'}</td>
                            <td>
                                <button class="btn btn-warning edit-btn" data-toggle="modal" data-target="#editModal">Edit</button>
                                <button class="btn btn-danger delete-btn">Delete</button>
                            </td>
                ...
                error: function(xhr) {
                    const errorMessage = xhr.responseJSON?.message || 'Error: An unexpected error occurred.';
        ...
        $(document).on('click', '.edit-btn', function() {
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    const errorMessage = xhr.responseJSON?.message || 'Error fetching data.';
                    alert('Error: ' + errorMessage);
                }
            });
        });
        ...
        $('#editBtn').on('click', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    row.find('td:nth-child(6)').text(response.item.tanggal_kembali || 'N/A');
                    alert(response.success);
                },
                error: function(xhr) {
                    const errorMessage = xhr.responseJSON?.message || 'Error: An unexpected error occurred.';
